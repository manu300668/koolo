...
function openAdminPanel(){
  ...
  adminColor.value="#ffffff"; // color inicial
  adminInstagram.value=localStorage.getItem("instagram")||"";
  renderAdminProducts();
}

// Admin productos con imágenes
function renderAdminProducts(){
  ...
  products.forEach((p,i)=>{
    ...
    d.innerHTML=`
      ...
      <input type="text" value="${p.img}" placeholder="URL imagen">
...
adminAddProduct.onclick=()=>{
  products.push({name:"Nuevo producto",price:0,img:"img/default.png"});
  renderAdminProducts();
}
...
adminSave.onclick=()=>{
  ...
  // Productos
  const inputs=adminProductsDiv.querySelectorAll("input");
  for(let i=0;i<products.length;i++){
    products[i].name=inputs[i*3].value;
    products[i].price=parseFloat(inputs[i*3+1].value);
    products[i].img=inputs[i*3+2].value;
  }
  drawProducts();
